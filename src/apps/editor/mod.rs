...
use crate::{apps::Application, shell::ShellContext};
...
impl TextEditor {
    ...
    pub fn save_file(&mut self) -> Result<(), &'static str> {
        if let Some(filename) = self.get_current_file() {
            ...
            self.file_ops
                .write_file(filename_str, &content_buffer[..content_size])?;
    ...
    pub fn save_file_as(&mut self, filename: &str) -> Result<(), &'static str> {
        ...
        let content_size = self.buffer.get_content_formatted(&mut content_buffer);
        self.file_ops
            .write_file(filename, &content_buffer[..content_size])?;
        self.set_current_file(filename);
        self.buffer.mark_saved();
        Ok(())
    }
                    ...
                    input::InputAction::Quit => {
                        if self.buffer.is_modified() {
                            self.ui.show_message(
                                "File modified. Save first (Ctrl+S) or force quit (Ctrl+Q)",
                                context,
                            );
                        } else {
                            self.running = false;
...
pub fn create_editor_command() -> impl Fn(&[&str], &mut ShellContext) {
    |args: &[&str], context: &mut ShellContext| {
        ...
        context
            .uart
            .puts("Commands: Ctrl+S (Save), Ctrl+Q (Quit), Ctrl+X (Force Quit)\n");
        context
            .uart
            .puts("Use arrow keys to navigate, type to edit.\n\n");
