...
//!
//! This module provides benchmarking capabilities for comparing TinyOS
//! performance against equivalent Linux operations. Focuses on Pi 4/5
...
impl LinuxComparisonSuite {
    ...
    fn benchmark_system_call_overhead(&mut self) {
        let tinyos_result = self.measure_tinyos_operation(
            "System Call",
            || {
            ...
            0,
        ); // No memory overhead for direct calls
    ...
    fn benchmark_memory_allocation(&mut self) {
        let tinyos_result = self.measure_tinyos_operation(
            ...
            || {
                ...
                let mut total = 0u64;
                for i in 0..64 {
                    // 64-byte block search
                    total = total.wrapping_add(i);
                }
                core::hint::black_box(total);
            },
            64,
        ); // 64-byte allocation overhead
    ...
    fn benchmark_cache_efficiency(&mut self) {
        let tinyos_result = self.measure_tinyos_operation(
            ...
            0,
        ); // Cache-optimized, minimal overhead
