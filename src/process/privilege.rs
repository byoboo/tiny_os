...
impl PrivilegeManager {
    ...
    pub fn transition_to_el1(&mut self, context: &ExceptionContext) -> EL0ToEL1Transition {
        ...
        if (esr >> 26) & 0x3F == 0x15 { // SVC instruction
            let syscall_num = context.gpr[8]; // ARM64 syscall convention (x8)
            let args = [
                context.gpr[0], context.gpr[1], context.gpr[2],
                context.gpr[3], context.gpr[4], context.gpr[5],
            ];
    ...
    pub fn transition_to_el0(&mut self, return_address: u64, return_value: u64) -> Result<(), &'static str> {
    ...
    pub fn validate_privilege(&mut self, required_level: PrivilegeLevel) -> Result<(), &'static str> {
