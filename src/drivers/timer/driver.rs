...
use super::hardware::{TimerHardware, frequency};
...
impl<H: HardwareVersion> TimerDriver<H> {
    ...
    pub fn set_timer_compare(&self, channel: u8, microseconds_from_now: u32) -> Result<(), DriverError> {
        if channel > 3 {
...
impl<const CHANNEL: u8, H: HardwareVersion> TimerChannel<CHANNEL, H> {
    ...
    pub fn set_compare(&self, microseconds_from_now: u32) -> Result<(), DriverError> {
        unsafe {
            (*self.driver).set_timer_compare(CHANNEL, microseconds_from_now)
        }
    }
    ...
    pub fn has_matched(&self) -> bool {
        unsafe {
            (*self.driver).has_timer_matched(CHANNEL)
        }
    }
