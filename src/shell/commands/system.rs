...
pub fn cmd_cow_stats(_args: &[&str], context: &mut ShellContext) {
    ...
    if let Some(cow_manager) = crate::memory::get_cow_manager() {
        ...
        if stats.cow_pages_count > 0 {
            let efficiency = (stats.memory_saved_bytes * 100) / 
                           (stats.cow_pages_count as u64 * 4096);
...
pub fn cmd_cow_test(_args: &[&str], context: &mut ShellContext) {
    ...
    if let Some(cow_manager) = crate::memory::get_cow_manager() {
        ...
        match cow_manager.register_page(test_phys, test_virt, 
                                      crate::memory::RegionType::UserData, 1) {
            Ok(()) => {
                context.uart.puts("PASS\r\n");
                tests_passed += 1;
            }
        ...
        match cow_manager.register_page(test_phys, test_virt + 0x1000, 
                                      crate::memory::RegionType::UserData, 2) {
            Ok(()) => {
                context.uart.puts("PASS\r\n");
                tests_passed += 1;
            }
        ...
        if cow_manager.is_cow_protected(test_phys) {
            ...
            context.uart.puts("FAIL (not protected after multiple refs)\r\n");
            tests_failed += 1;
        }
